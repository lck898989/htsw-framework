...
class ShockUtils extends BaseClass {
    ...
    private mapPoss: any[] = [new egret.Point(0, 3), new egret.Point(0, 0), new egret.Point(0, -2)];
    private spritePoss: any[] = [new egret.Point(5, 0), new egret.Point(-5, 0), new egret.Point(5, 0)];
    private _shockPoss: any[];
    ...
    private onShockEnter(time: number): void {
        const maxCount: number = this._shockLength * this._repeatCount;
        if (this._shockCount >= maxCount) {
            this.stop();
            return;
        }
        const index: number = this._shockCount % this._shockLength;
        const pos: egret.Point = this._shockPoss[index];
        if (this._target) {
            if (this._type != this.MAP) {
