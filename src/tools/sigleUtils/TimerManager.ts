...
class TimerManager extends BaseClass {
    ...
    private _delHandlers: TimerHandler[];
     ...
     *
     */
    public doTimer(delay: number, repeatCount: number, method: Function
        ,          methodObj: any, complateMethod: Function = null, complateMethodObj: any = null): void {
     ...
     *
     */
    public doFrame(delay: number, repeatCount: number, method: Function, methodObj: any
    ...
    private onEnterFrame(): void {
        ...
        for (let i: number = 0; i < this._count; i++) {
            const handler: TimerHandler = this._handlers[i];
            const t: number = handler.userFrame ? this._currFrame : this._currTime;
        ...
        while (this._delHandlers.length) {
            const handler: TimerHandler = this._delHandlers.pop();
            this.remove(handler.method, handler.methodObj);
        }
    }

    private create(useFrame: boolean, delay: number, repeatCount: number
        ...
        // 创建
        const handler: TimerHandler = ObjectPool.pop("TimerHandler");
